...
function ruta_editar(idruta){
    vistacargando('M','Espere...'); // mostramos la ventana de espera
    $.get("/gestionAccesos/ruta/"+idruta+"/edit", function (data) {
        console.log(data);
        $('#descripcion').val(data.resultado.descripcion);
        $('#ruta').val(data.resultado.ruta);
        $('#icon_ruta').val(data.resultado.icono);
        $('#icon_ruta_btn').html(`<i class="${data.resultado.icono}"></i>`);
        $('.option_menu').prop('selected',false); // deseleccioamos todas las zonas del combo
        $(`#cmb_menu option[value="${data.resultado['idmenu']}"]`).prop('selected',true); // seleccionamos la zona que pertenece el sector seleccionado 
        $("#cmb_menu").trigger("chosen:updated");
    ...
    $('#method_Ruta').val('PUT'); // decimo que sea un metodo put
    $('#frm_Ruta').prop('action',window.location.protocol+'//'+window.location.host+'/gestionAccesos/ruta/'+idruta);
    $('#btn_ruta_cancelar').removeClass('hidden');

    $('html,body').animate({scrollTop:$('#administradorRuta').offset().top},400);
}

$('#btn_ruta_cancelar').click(function(){
    $('#descripcion').val('');
    $('#ruta').val('');
    $('#icon_ruta').val('fa fa-circle-o');
    $('#icon_ruta_btn').html(`<i class="fa fa-circle-o"></i>`);
    $('.option_menu').prop('selected',false); // deseleccionamos las zonas seleccionadas
    $("#cmb_menu").trigger("chosen:updated");

    $('#method_Ruta').val('POST'); // decimo que sea un metodo put
    $('#frm_Ruta').prop('action',window.location.protocol+'//'+window.location.host+'/gestionAccesos/ruta/');
    $(this).addClass('hidden');

});

//ELIMINAR RUTA
function btn_eliminar_ruta(btn){
